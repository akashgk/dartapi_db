...
void main() {
  ...
  group('SqliteDatabase - CRUD', () {
    ...
    test('update modifies matching rows', () async {
      await db.insert('users', {'name': 'Alice', 'email': 'a@a.com'});
      await db.update('users', {'name': 'Alicia'}, where: {'email': 'a@a.com'});
    ...
    test('rawQuery executes DDL', () async {
      ...
      await db.rawQuery("INSERT INTO items (label) VALUES ('hello');");
  ...
  group('SqliteDatabase - transaction', () {
    ...
    test('transaction returns value from callback', () async {
      final id = await db.transaction((tx) async {
        final result = await tx.insert('users', {
          'name': 'Charlie',
          'email': 'c@c.com',
        });
  ...
  group('MigrationRunner', () {
    ...
    test(
      'creates _dartapi_migrations table and applies pending files',
      () async {
        ...
        expect(
          applied,
          containsAll(['0001_create_items.sql', '0002_add_price.sql']),
        );
    ...
    test('applies only new migrations on second run', () async {
      ...
      File(
        '${tempDir.path}/0002_add_price.sql',
      ).writeAsStringSync('ALTER TABLE items ADD COLUMN price REAL DEFAULT 0;');
      await runner.migrate();
    ...
    test(
      'throws StateError when migrations directory does not exist',
      () async {
