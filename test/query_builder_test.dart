...
void main() {
  ...
  group('QueryBuilder — .get()', () {
    ...
    test('where notEquals', () async {
      final result =
          await db.query('users').where('role', notEquals: 'user').get();
      expect(result.rows.length, equals(2)); // admin + editor
    });

    test('where greaterThan', () async {
      final result =
          await db.query('users').where('age', greaterThan: 28).get();
    ...
    test('where greaterThanOrEqual', () async {
      final result =
          await db.query('users').where('age', greaterThanOrEqual: 30).get();
      expect(result.rows.length, equals(2)); // Alice(30), Carol(35)
    });

    test('where lessThanOrEqual', () async {
      final result =
          await db.query('users').where('age', lessThanOrEqual: 28).get();
      expect(result.rows.length, equals(2)); // Bob(25), Dave(28)
    });

    test('where like — suffix pattern', () async {
      final result =
          await db.query('users').where('email', like: '%@corp.com').get();
    ...
    test('where isNull = true matches null column', () async {
      ...
      final result =
          await db.query('users').where('name', isNotNull: true).get();
      expect(result.rows.length, greaterThanOrEqualTo(4));
    });

    test('where isNotNull filters out null values', () async {
      final result =
          await db.query('users').where('role', isNotNull: true).get();
      expect(result.rows.length, equals(4));
    });
    ...
    test('orderBy descending', () async {
      final result =
          await db.query('users').orderBy('age', ascending: false).get();
      final ages = result.rows.map((r) => r['age'] as int).toList();
    ...
    test('limit + offset — page 2 of 2', () async {
      final page1 =
          await db.query('users').orderBy('id').limit(2).offset(0).get();
      final page2 =
          await db.query('users').orderBy('id').limit(2).offset(2).get();
  ...
  group('QueryBuilder — .first()', () {
    ...
    test('returns null when no row matches', () async {
      final row =
          await db.query('users').where('name', equals: 'Nobody').first();
      expect(row, isNull);
    });
  });
